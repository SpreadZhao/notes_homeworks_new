...
data segment
	...
	arr		dw 1024,24,1,0,60,66,1,24,50,100
			db str_end
	len		db 10
	div_n	dw 10
	...
	ui_title db 218, '-------------------------------------', 191, cr, lf
			 db 179, '                                     ', 179, cr, lf
			 db 179, '  Find 2nd Max unsigned int_16 v1.0  ', 179, cr, lf
			 db 179, '                                     ', 179, cr, lf
			 db 192, '-------------------------------------', 217, cr, lf
			 db cr, lf
...
output proc
	call	print_result
	lea		si, arr + 16 ;[TODO]16 only works when len == 10
	mov		bx, 0
	mov		ax, [si]
	o_l_loop1:
		xor		dx, dx
