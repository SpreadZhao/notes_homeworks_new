...
这部分对应[[数据管理技术#6 13 Hadoop设置|第一次实验的Hadoop设置]]，并且内容都很相似。首先打开`/home/work/_app/hbase-2.4.14`这个根目录(以下记为`<hbRoot>`)下的`conf/`目录，然后编辑其中的`hbase-env.sh`文件，将其中的几项修改成这样： ^706f2d
