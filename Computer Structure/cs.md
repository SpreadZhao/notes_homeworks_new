...
>   T = 1 + 10% * 50 = 6(ns)
>
>   #question `90% * 1 + 10% * (1 + 50)`为啥不是`90% * 1 + 10% * (1 + 50 + 1)`呢？应该是1级Cache没命中，但也访问了，用了1，然后从MM中搬数据，用了50，最后访问1级Cache，用了1
>
> * L1 + L2
>
...
#question 这个程序在emu8086上不能一直循环，因为执行的时候还有一个cx，在不断递减，当递减到0的时候，会跳出循环，就像下图这样。为什么会有这个cx?
...
#question 那么这种设计方式，是不是要走地址线两次才能得到一个物理地址呢？
...
> #question  联想一下c语言的数组，聪明的你一定能发现规律的！这里只留下一个问题：*为什么有时候后面没有+xxh这种，有时候有呢？*
