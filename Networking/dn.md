...
一个datagram传过来，如果我这个frame装不下，那咋办？datagram的total length字段已经规定了，一个数据报的最大长度就是65535个字节。但是某些链路层的frame的数据段根本不允许有这么大(比如[[#12.4.1 PPP Framing|ppp协议中的Frame]])，那么这个时候就需要对datagram切上记到，分散到若干个frame中。一旦开始切了，我们就需要考虑很多问题：
