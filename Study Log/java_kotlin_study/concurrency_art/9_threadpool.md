...
- 核心线程池数。这个是线程池刚被创建，任务还不多的时候，可以同时运行的最大线程数。比如如果是4，我提交了一个任务，1个线程运行来执行；然后我又提交了一个，这个时候会再创建一个线程运行（**需要获取全局锁**）；当4个线程都在执行任务时，我如果再提交，那么就不会再创建新的线程了。
- 任务队列：就是存放任务的队列。还是刚刚4个核心线程的例子。如果第五个任务被提交，此时不会再创建新的线程，会将任务入队。
...
如果第五个任务来的时候，有空闲的核心线程。此时任务会入队还是直接被其中一个线程执行？

通过上面的解释，我们可以看出来，比较影响性能的就是这两个创建线程的过程，这个过程[[#^ca5c6d|需要获取全局锁]]。因此，如果没有核心线程数只有最大线程数的话，全局锁获取就会很频繁。比如一个脑瘫写的代码，直接创建了100个线程的线程池，但是他提交的任务就星崩几个，而每次提交，调用execute，都要获取一次全局锁。这样就太浪费了。所以，这里将两个数字拆开，把任务队列放到中间，这样绝大多数情况都会走入队的操作，这样就不用获取全局锁了。
...
- 一个线程要结束了，最后会做一些收尾工作了。此时线程[[#^e8989f|不会再运行新任务]]，但是因为它没有真正停止，所以此时workerCount其实还是把它给算上了的。
...
如果这里不行，那就是超过了核心线程数，应该入队了。所以这里[[#^52b263|检查一下线程池是否任然在运行]]，如果在运行就会尝试入队。入队成功了，就完了吗？没有。在入队成功之后，可能会发生下面的情况：
