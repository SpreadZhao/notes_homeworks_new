...
> - 判断3和4要不要重排。
> 
> 也就是每两个指令为单位进行重排序。注意，这是由于volatile有这样的规则，所以要检测的。不过这个问题深究下去也没什么含金量就是了。

- [ ] #TODO tasktodo1715333911391 现在看来，上面的这个表格其实是不准确的。比如第一个指令是普通读，第二个指令是volatile读。这个时候其实是可以重排序的，因为普通读并不会影响内存的数据（大概吧，如果你倒着排，随机排，算数据依赖，，感觉也都还行），后面的那个question也说明了这一点：[[#^4bb4ad]]。所以还是和[[Study Log/java_kotlin_study/concurrency_art/3_3_sequential_consistency#^b68769|tasktodo1715333769430]]一样，还是要确定JMM原文中是怎么规定的。 🔼 ➕ 2024-05-10
...
- 在它前面插入了一个StoreStore屏障。这保证了必须前面的普通写已经刷新到内存之后，volatile写才能执行。这也就禁止了重排序。
	- [?] #question/coding/theory *这里为什么插入的是StoreStore而不是StoreLoad？前面如果是普通读的话，不就又可以重排序了吗？但实际上我不想让它重排序呀？StoreLoad有其它三个屏障的所有功能，为什么不插入这个？再或者，你像下面volatile读那样，在StoreStore后面跟上一个LoadStore也行呀？你为啥不跟呢？* ^4bb4ad
