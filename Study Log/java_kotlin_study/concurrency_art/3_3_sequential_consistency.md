...
> [!comment]- 单线程里就别重排序啦
> 这里要纠正一下，通过下面的图也能看出来，顺序一致性模型并不是让单线程不允许重排序，而是你把所有线程的指令拎出来，拍到一块儿去之后，经过重排序之后，这些指令的时间顺序要和单线程中的时间顺序一样。具体看下面的图就知道了。
...
- [ ] #TODO 这里我突然想到了一个问题：在Lock中，比如你调用了lock.lock()，那么后面的代码就是获得了锁之后才能执行的了。那么，java是怎么保证lock里面的指令不会被重排序到lock外面的呢？言外之意，Java的lock，或者其底层的AQS里面的那个volatile，就能保证吗？？？？。。。。卧槽，好像还真能。根据后面的volatile的定义，就能保证重排序了。这个问题需要总结一下好好说一说。 ➕ 2024-03-10 🔼 
