...
通过这张图我们发现，就像我们之前说的那样，tryLock()只会走非公平的实现nonfairTryAcquire()。**想要调用到公平锁的tryAcquire()**，只能用lock()。而[[#^fb346a|之前]]我们说过，公平锁在发现没有竞争的时候要进去排队。但是，公平锁的tryLock()调用的居然也是非公平的实现。理论上，如果是公平锁的tryLock()，应该是只要发现有人在排队，或者CAS失败就返回false。但是不知道为什么jdk8没提供这样的实现，而是打破了这个规则，转而让用另一个版本的tryLock()来做这件事。
