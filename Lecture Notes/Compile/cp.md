...
程序中的变量存放在内存中，在计算的时候需要加载到寄存器中，就像[[Lecture Notes/Computer Structure/cs#2.1 Overview|计组笔记中的这里]]提到的一样。所以对于计算机来说，比如`x = a + b;`这样的代码就必须经过翻译才能够被计算机认识，才知道去哪里寻找这个变量，怎么去计算。
...
![[Lecture Notes/Compile/resources/Pasted image 20221018105859.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221018112112.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-10-18 12.08.38.excalidraw.png|350]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221018125424.png]]

* **语法分析**

其实语法分析和文法分析是一回事，主要的工作是生成分析树[[#2.10 CFG Tree|CFG]]。

![[Lecture Notes/Compile/resources/Pasted image 20221018125433.png]]

* **语义分析**

就是上面所说的某个变量应该存放在哪个位置。也就是**检查语法正确的句子语义是否有问题**。比如把整形赋值给浮点型，那么**通知**隐式转换(通知的意思是不做，告诉CPU你去做)就是这部分做的事。

![[Lecture Notes/Compile/resources/Pasted image 20221018125314.png]]

* **中间代码生成**

语法和语义都正确的话，就可以真正开始逐个翻译了。这部分做的事情就是将分析树这种不能执行的结构翻译成计算机能够差不多认出来的可执行序列。之所以是差不多是因为这个语言还不能完全执行(比如汇编)。而代码优化也需要翻译成中间代码后才能优化。

![[Lecture Notes/Compile/resources/Pasted image 20221018125221.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221018125827.png]]

* **目标代码生成**

![[Lecture Notes/Compile/resources/Pasted image 20221018130028.png]]

* **符号表管理**

在前面其实已经见过这个表了：

![[Lecture Notes/Compile/resources/Pasted image 20221018141548.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221018142751.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221018143127.png]]

## 0.3 练习题

#homework preview

![[Lecture Notes/Compile/resources/Pasted image 20221018154150.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221018154208.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221018154225.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221018154249.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221018154320.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221018154348.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221018154411.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221018154425.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221106114609.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221106114410.png]]
...
#question/class 这里对于$\alpha$的概念：是不是$\alpha$表示的是左边所有东西构成的集合？还是任何一个比如`<动词短语>`都可以叫做$\alpha$变量？如果是前者的话，那为什么要用$\in$而不是$\subseteq$?
...
L(G) = {$\omega$ | S $\Rightarrow^*$ $\omega$, $\omega$ $\in$ ${V_T}^*$} ( #question/class 这里$V_T$是否加上星号存疑 )
...
![[Lecture Notes/Compile/resources/Pasted image 20221017111600.png]] ^bingde
...
![[Lecture Notes/Compile/resources/Drawing 2022-11-22 15.19.24.excalidraw.png|400]]
...
![[Lecture Notes/Compile/img/cfgt.png|200]]
...
![[Lecture Notes/Compile/img/cfgt2.png|100]]
...
![[Lecture Notes/Compile/img/cfgt3.png|300]]

这个文法的分析树：

![[Lecture Notes/Compile/img/cfgt4.png|300]]
...
![[Lecture Notes/Compile/img/cfgt5.png|300]]
...
![[Lecture Notes/Compile/img/cfgt6.png|300]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221106112614.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221106113558.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20230224140158.png]]
...
* 如果克林闭包不好用，可以这样：
  ...
  ![[Lecture Notes/Compile/resources/Pasted image 20221106121350.png]]
* 如果某个部分可以没有，那么我们通常的做法是$r | \epsilon$，而如今可以这么写：r?

  ![[Lecture Notes/Compile/resources/Pasted image 20221106121458.png]]
* a|b|c等价于\[abc\]，而\[0-9a-z\]等价于那一大串
* \[^r\]等价于$\Sigma - L(r)$

  ![[Lecture Notes/Compile/resources/Pasted image 20221106121644.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221017112429.png]]
...
![img](Lecture%20Notes/Compile/img/fa.png)
...
![[Lecture Notes/Compile/resources/Pasted image 20221017112732.png]]
...
![img](Lecture%20Notes/Compile/img/fa2.png)
...
![img](Lecture%20Notes/Compile/img/dfa.png)

* 这里面一共有四个状态：0, 1, 2, 3，它们构成了**状态集合S**
* 能输入的符号有a, b，它们构成了**字母表集合$\Sigma$**
* 然后我们画一张表格：

  ![[Lecture Notes/Compile/resources/Pasted image 20221017112952.png]]
...
![img](Lecture%20Notes/Compile/img/nfa.png)
...
![img](Lecture%20Notes/Compile/img/nfa2.png)
...
![img](Lecture%20Notes/Compile/img/nfa3.png)

这里要注意：**三个状态都是终态**，如果在状态A结束表示若干个0；在状态B结束表示若干个0加上若干个1，因为一但跳到状态B，就不再接收0了；在状态C结束表示若干个0连上若干个1连上若干个2

#question/class 线上写的是0,1；1,2；0,1,2，为啥要写3个数呢？只写上1；2；2不是也可以吗？可能是因为这里要构造NFA，所以硬加上去的？
...
![[Lecture Notes/Compile/img/snfa.png|200]]

2. 然后是字母表，字母表中的每一个字母，都是一个正则表达式，它对应的NFA是

![[Lecture Notes/Compile/img/anfa.png|200]]

3. 然后是俩表达式连到一起，对于r = $r_1r_2$，它对应的NFA是

![[Lecture Notes/Compile/img/rrnfa.png|300]]

4. 然后是两个表达式或运算，对于r = $r_1|r_2$，它对应的NFA是

![[Lecture Notes/Compile/img/rnfa.png|300]]

5. 最后是克林闭包，对于r =$r_1^*$，它的NFA是

![[Lecture Notes/Compile/img/knfa.png|200]]

由这些基本规则，我们就可以由正则表达式来构建NFA了。比如：r = $(a|b)^*abb$

一开始，给一个初始状态和终止状态，然后把表达式写上去：

![[Lecture Notes/Compile/img/z1.png]]

很显然，这个表达式可以拆成连接的形式，根据第3个规则，就可以：

![[Lecture Notes/Compile/img/z2.png]]

然后，第一个是个克林闭包，也就是把**两个点合成一个点，然后自旋**

![[Lecture Notes/Compile/img/z3.png]]

最后把这个或也拆开，就变成了俩自旋

![[Lecture Notes/Compile/img/z4.png]]

### 3.5.2 NFA -> DFA

用程序去描述NFA(将NFA写成程序，让这个程序去识别字符串)通常是很困难的，而且效率会很低下(通常会用DFS，BFS，递归等技术)。所以我们需要将NFA转换成DFA再去用程序描述。

![[Lecture Notes/Compile/resources/Pasted image 20221108181104.png]]
...
![img](Lecture%20Notes/Compile/img/nd1.png)
...
![img](Lecture%20Notes/Compile/img/nd2.png)

这个初始状态在遇到a的时候，会进入A或者B，**那既然是俩状态，合到一起就可以了！**

![img](Lecture%20Notes/Compile/img/nd3.png)

那接下来咋办呢？看A和B：A和B收到a时，会进入{A, B}或者$\Phi$，因为空集不算，那还是自己

![img](Lecture%20Notes/Compile/img/nd4.png)

AB收到b时会进入$\Phi$和{B, C}，因此

![img](Lecture%20Notes/Compile/img/nd5.png)

然后，BC收到b会进入自己；收到c会进入CD

![img](Lecture%20Notes/Compile/img/nd6.png)

别忘了，CD还会接收c变成自己！

![img](Lecture%20Notes/Compile/img/nd7.png)

检验，NFA和DFA能接收的串都是：$aa^*bb^*cc^*$(这里是因为至少要有一个a或b或c，也可以写成$a^+b^+c^+$)

接下来，再看一个例子，这个例子带空边

![img](Lecture%20Notes/Compile/img/nd8.png)
...
![[Lecture Notes/Compile/img/nd9.png|200]]

然后看ABC接收0之后只能变成自己，那么

![[Lecture Notes/Compile/img/nd10.png|200]]

ABC接收1之后会进入BC，并且BC中也含终态，所以

![[Lecture Notes/Compile/img/nd11.png|200]]

ABC接收2之后会进入C，C是终态

![[Lecture Notes/Compile/img/nd12.png|200]]

最终都分析完之后，DFA构建好了

![[Lecture Notes/Compile/img/nd13.png|300]]
...
![[Lecture Notes/Compile/img/w1.png|300]]
...
![img](Lecture%20Notes/Compile/img/w4.png)
...
![[Lecture Notes/Compile/resources/Drawing 2022-11-22 14.37.10.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221105221228.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221105221252.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221105221316.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221105221328.png]]

### 3.8.2 自动机

#homework automata

![[Lecture Notes/Compile/resources/Pasted image 20221106132715.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221106132734.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221106132752.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221106132809.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221106132842.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221106132858.png]]

### 3.8.3 RE -> FA

#homework RE->FA

![[Lecture Notes/Compile/resources/Pasted image 20221122141129.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221122141148.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221122141200.png]]

# 4. Syntax Analysis

语法分析器扫描的实际上就是词法分析结束后的结果：

![[Lecture Notes/Compile/resources/Pasted image 20221122152451.png]]
...
![img](Lecture%20Notes/Compile/img/td1.png)
...
![img](Lecture%20Notes/Compile/img/td2.png)
...
![img](Lecture%20Notes/Compile/img/td3.png)
...
![img](Lecture%20Notes/Compile/img/ld1.png)

然后从Start Symbol开始，也就是E，E只能按规则1替换

![img](Lecture%20Notes/Compile/img/ld2.png)

然后这时还没有id，所以要接着替换。从**最左边**开始，把T按着规则3替换

![img](Lecture%20Notes/Compile/img/ld3.png)

继续， 把F按着规则5替换。这时问题来了：F既可以替换成(E)也可以替换成id。那么替换哪个呢？我们**输入指针**指向的是id，那自然要就近来。所以要把F替换成id，自然也就匹配了

![img](Lecture%20Notes/Compile/img/ld4.png)

然后，**输入指针**自然也要往后移一位了

![img](Lecture%20Notes/Compile/img/ld5.png)

此时最左边的Nonterminal是T'，自然要替换它了。它能替换成\*FT'或者$\epsilon$，而现在是个加号。很显然，T'不可能以\*开头，那么我们只能将T'替换成$\epsilon$了

![img](Lecture%20Notes/Compile/img/ld6.png)

此时最左边的是E'，它能替换成+TE'或者$\epsilon$，显然以加号开头的这个正是我们想要的，那么就这么替换

![img](Lecture%20Notes/Compile/img/ld7.png)

然后输入指针后移一位

![img](Lecture%20Notes/Compile/img/ld8.png)

然后开始替换T，只能替换成FT'

![img](Lecture%20Notes/Compile/img/ld9.png)

然后自然是把F替换成id了，并且后移一位输入指针

![[Lecture Notes/Compile/img/ld10.png|200]] ![[Lecture Notes/Compile/img/ld11.png]]

替换T'，因为这里是星号，所以替换成*FT'并后移输入指针

![[Lecture Notes/Compile/img/ld12.png|200]] ![[Lecture Notes/Compile/img/ld13.png]]

然后F替换成id往下走，因为已经完了，输入指针指向的是空串，所以只要能换成$\epsilon$就换了。最终

![[Lecture Notes/Compile/img/ld14.png|200]] ![[Lecture Notes/Compile/img/ld15.png]]
...
#question/class #idea 这里仅仅是个人猜想，有待证明
...
* 当$\alpha$能推出空串，但是$\beta$不能时，我们先分别写一下他们的可选集
  ...
  \end{align}
  $$
#question/class PPT中只给了第一条，这第二条不需要吗难道？
...
![[Lecture Notes/Compile/resources/Pasted image 20221017121121.png|400]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221017121229.png|400]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221017121300.png|400]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221017121320.png|400]]

第三句中，T'后面出现的应该包括T后面出现的所有；另外T'能变空串，所以F后面出现的也应该包括T后面出现的所有

![[Lecture Notes/Compile/resources/Pasted image 20221017121513.png|400]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221017121603.png|400]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-09 21.06.00.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-09 21.14.52.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-09 21.17.48.excalidraw.png]]

现在回头看看，我们出问题的E+T在哪里：

![[Lecture Notes/Compile/resources/Drawing 2022-12-09 21.19.33.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-09 21.27.35.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221209221009.png|300]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-10 12.56.19.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-10 13.02.56.excalidraw.png]]

这样我们就可以很清晰地看出自己走到了哪一步了。到这里，自动机还是不太对计算机友好，我们之前不是**把所有的或都拆开**了吗？因此我们要更加优化一下：

![[Lecture Notes/Compile/resources/Drawing 2022-12-10 13.05.40.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221210131233.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221210131847.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221210135107.png|150]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221210135350.png|150]]

经过不断运算，我们最终能求出所有的集合，而且最终形成的一定是个DFA：

![[Lecture Notes/Compile/resources/Pasted image 20221210135437.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221214173830.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221214174256.png]]

* 按着这个节奏，就可以把整张表的移进部分都填出来：

![[Lecture Notes/Compile/resources/Pasted image 20221214174333.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221214174745.png]]

* 按照这个规则，我们也可以把所有的规约操作也填好：

![[Lecture Notes/Compile/resources/Pasted image 20221214174818.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221122182733.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221122182827.png|200]]

![[Lecture Notes/Compile/resources/Pasted image 20221122182843.png|300]]

![[Lecture Notes/Compile/resources/Pasted image 20221122182909.png|300]]

![[Lecture Notes/Compile/resources/Pasted image 20221122182921.png|300]]

![[Lecture Notes/Compile/resources/Pasted image 20221122182937.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221122183145.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221122183202.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221122183211.png]]

### 4.5.2 Top-down parsing & LL1

#homework Top-down parsing and LL1

![[Lecture Notes/Compile/resources/Pasted image 20221203193327.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221203193339.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221203193358.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221203193408.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221203193450.png]]

### 4.5.3 Down-top parsing & SLR1

#homework Down-top parsing & SLR1

![[Lecture Notes/Compile/resources/Pasted image 20221212181030.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221212181042.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221212181054.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221212181104.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221212181112.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221212181124.png|300]] ![[Lecture Notes/Compile/resources/Pasted image 20221212181135.png|300]]

![[Lecture Notes/Compile/resources/Pasted image 20230226142202.png]]

![[Lecture Notes/Compile/resources/Pasted image 20221212181144.png]]

![[Lecture Notes/Compile/resources/Pasted image 20230226142339.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221227153545.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221227161323.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-29 13.58.11.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-29 14.09.14.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-29 14.14.45.excalidraw.png]]

> 这里的(1)就是`trip()`函数第一次被调用返回的结果。它生成一个三元式`(1) (*, b, c)`。要注意这里的b和c还是code哦!

* 接下来就是将E+E规约成E了，并按照第二条产生式的语义规则继续生成三元式：

![[Lecture Notes/Compile/resources/Drawing 2022-12-29 14.18.33.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20221231141219.png]]

> #poe 注意分析树和语法树的区别！！！
> 
> ![[Lecture Notes/Compile/resources/SmartSelect_20230226_135916_Samsung Notes.jpg]]

于此同时，也可以写出它的三元式和四元式：

![[Lecture Notes/Compile/resources/Pasted image 20221231141247.png]] ![[Lecture Notes/Compile/resources/Pasted image 20221231141302.png]]

我们能发现，这棵树其实和三元式，和四元式都是对应的。因此我们可以给这棵树的结点添加一些信息，告诉它这棵树的结果存放位置：

![[Lecture Notes/Compile/resources/Pasted image 20221231141508.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2022-12-31 17.03.28.excalidraw.png]]

首先要规约的就是这个int，将它规约成T之后，就知道了这个T的type和width：

![[Lecture Notes/Compile/resources/Drawing 2022-12-31 17.14.06.excalidraw.png]]

之后就是将array的部分总共再规约成一个T，那么此时也就是调用`array()`函数来构建整个数组的信息了；另外下一步是将最右边的int规约成T，就一起做了。

![[Lecture Notes/Compile/resources/Drawing 2022-12-31 17.17.39.excalidraw.png]]

下一步是将id:T规约成D，那么这个时候终于用到第二条产生式了！此时就真正开始填表操作：

![[Lecture Notes/Compile/resources/Drawing 2022-12-31 17.21.42.excalidraw.png]]
...
> 
> ![[Lecture Notes/Compile/resources/Pasted image 20230102203517.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20230103113929.png]] ![[Lecture Notes/Compile/resources/Pasted image 20230103114000.png]]

然后要规约这个N，这代表又新建了一张表，它的父亲是t1，将这张新表也压入栈中。这张新表中存的就是`sort`函数中的符号了。

![[Lecture Notes/Compile/resources/Pasted image 20230103114146.png]] ![[Lecture Notes/Compile/resources/Pasted image 20230103114159.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20230103120944.png]] ![[Lecture Notes/Compile/resources/Pasted image 20230103120951.png]]

在经过若干步后，我们会规约完`x : int`这句话。那么此时依然是要填符号表了，填完符号表，依然要更改`offset`的栈顶：

![[Lecture Notes/Compile/resources/Pasted image 20230103121202.png]] ![[Lecture Notes/Compile/resources/Pasted image 20230103121210.png]]

再之后，就是里面的proc的规约了。此时依然会先规约出N，那么就会再建一个符号表，他的父亲是t2，并且将它压入栈中：

![[Lecture Notes/Compile/resources/Pasted image 20230103121355.png]]

接下来自然就是把变量`i`填入t3中了：

![[Lecture Notes/Compile/resources/Pasted image 20230103121614.png]]

之后有一个`read(a)`，这里的操作可能是生成三地址码之类的，我们这里就忽略了。当这句执行完之后，这代表着一个D就规约出来了。此时应该按照第5条产生式的语义规则，来出栈了！同时我们也要给t3的父亲t2登记上t3的信息，作为符号表中的一项：

![[Lecture Notes/Compile/resources/Pasted image 20230103122135.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20230103122334.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20230106140145.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2023-01-06 15.38.22.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20230106174723.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2023-01-06 21.13.21.excalidraw.png]]

前几步都是正常的规约，这里就直接跳过了：

![[Lecture Notes/Compile/resources/Drawing 2023-01-06 21.46.16.excalidraw.png]]

此时我们要将arr\[E规约成E'，那么就用到了第四条产生式的语义规则了。此时记住我们最重要的目的：**算v1**！其他所有的操作，其实都是在为我们后续算v服务。按着语义规则走下去，我们能得到如下结果：

![[Lecture Notes/Compile/resources/Drawing 2023-01-06 21.54.36.excalidraw.png]]

v是在place属性中存的，所以我们每次最重要的结果就是得到新的规约出来的东西的place属性。接下来是规约j+y的过程，也直接跳过了：

![[Lecture Notes/Compile/resources/Drawing 2023-01-06 21.57.47.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2023-01-06 22.04.41.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Drawing 2023-01-06 22.14.22.excalidraw.png]]
...
![[Lecture Notes/Compile/resources/Pasted image 20230107115258.png]]
