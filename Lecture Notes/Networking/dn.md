...
![img](Lecture%20Notes/Networking/img/fc.png)
...
![img](Lecture%20Notes/Networking/img/sp.png)
...
![img](Lecture%20Notes/Networking/img/hd.png)

能双向流，但是却不能完全双向流。也就是说在一个时刻只能有一个方向。不能有一个时刻又往左又往右。比如对讲机。你说话的时候就不能听别人说；听别人说的时候你也不能说话。

##### 1.5.2.2 Full-Duplex

![img](Lecture%20Notes/Networking/img/fd.png)
...
![img](Lecture%20Notes/Networking/img/p2p.png)

也就是经常见到的p2p。这个Link是被两个Node独享的

#### 1.7.2 Multipoint

![img](Lecture%20Notes/Networking/img/mt.png)
...
![[Lecture Notes/Networking/img/mesh.png|center|400]]
...
![img](Lecture%20Notes/Networking/img/star.png)
...
#question/class 那既然Star的Hub可以是Bus，那这不就和Bus Topology一模一样了吗？

![img](Lecture%20Notes/Networking/img/bus.png)

#### 1.8.4 Ring Topology

如果把这根线变成一个圈，就能用Simplex去模拟Duplex了。因为就算不能往另一边走，转一圈回来也能到。

![img](Lecture%20Notes/Networking/img/ring.png)
...
![img](Lecture%20Notes/Networking/img/lan.png)
...
![img](Lecture%20Notes/Networking/img/i1.png)

然后是一个很复杂的Internet，由4个WAN和3个LAN组成。

![img](Lecture%20Notes/Networking/img/i2.png)

#### 1.9.3 The Internet

注意internet和Internet的区别。小写字母是上面讨论的；而大写字母是全世界最大的，将所有的小internet连接起来组成的唯一的Internet。下面就来分析以下这个全世界最大的网络。

![img](Lecture%20Notes/Networking/img/Internet.png)
...
![img](Lecture%20Notes/Networking/img/In.png)
...
![img](Lecture%20Notes/Networking/img/layer.png)
...
![img](Lecture%20Notes/Networking/img/lc.png)
...
![img](Lecture%20Notes/Networking/img/lc2.png)
...
#question/class 这里Source host和Destination host拿到的是一样的object吗？不是经过了路由器的改变吗(为了适应不同的LAN，采用了不同的Protocol)？或者换一个问法，数据从Source host传下来，当到达网络层的时候，确实这时候数据是没变过的，但是当到达Data link层的时候，之后的传递过程中在路由器那儿发生了改变，也就是**从Source host的Data link层发出的数据和Destination host的Data link层接收的数据是不一样的**，那么又怎么保证这个接收的数据再向上传递的时候，和Source host那边又变回一致了？

> 数据在两个网络层其实是一样的，没有改变，只不过这里路由器采用了两套不同的协议，真正要传的东西其实是一样的，只是额外附加传的东西会有不同(因为协议不同)。

![img](Lecture%20Notes/Networking/img/lc3.png)
...
![img](Lecture%20Notes/Networking/img/z1.png)

是因为：**这俩玩意儿根本就不能通信！！！**协议不一样那咋通信？所以我们才需要断开。

![img](Lecture%20Notes/Networking/img/z2.png)
...
![img](Lecture%20Notes/Networking/img/fl.png)
...
![img](Lecture%20Notes/Networking/img/nl.png)

![img](Lecture%20Notes/Networking/img/dll.png)
...
#question/class 数据链路层和网络层都提到了路由器的职责，那么这个功能到底是包括在网络层还是包括在数据链路层？或者是这俩其实是不相同的功能，只不过有些类似而已？

![img](Lecture%20Notes/Networking/img/nll.png)
...
![img](Lecture%20Notes/Networking/img/ed.png)
...
![img](Lecture%20Notes/Networking/img/ad.png)
...
![img](Lecture%20Notes/Networking/img/osi.png)
...
![img](Lecture%20Notes/Networking/img/sig.png)

### 3.2 Periodic Analog Signals

#### 3.2.1 Sine Wave

![img](Lecture%20Notes/Networking/img/sin.png)
...
![img](Lecture%20Notes/Networking/img/hz.png)

#idea **频率就是1秒钟扑腾的次数。** ^84ab18

**常用的频率和周期单位：**

![img](Lecture%20Notes/Networking/img/hz2.png)

接下来我们玩点儿极限的：如果频率变成0会咋样？也就是周期无穷大，那么这个波就是从头到尾永远是一个值。那我们就称这个为**直流信号**。它在数据传递的时候鸟用没有。如果频率是无穷大的话，周期就是0，也就代表它只要一瞬间就有很大的变化。

#### 3.2.3 Phase

![img](Lecture%20Notes/Networking/img/phase.png)

#### 3.2.4 Wavelength

![img](Lecture%20Notes/Networking/img/wl.png)
...
![img](Lecture%20Notes/Networking/img/cp.png)

那么我们可以将它分解，分解的结果如下：

![img](Lecture%20Notes/Networking/img/cp2.png)

**我们能看到，组成这个复杂波的正弦波的频率就三种，所以是离散的**。

另外，如果是一个非周期的复杂波的话：

![img](Lecture%20Notes/Networking/img/cp3.png)
...
![img](Lecture%20Notes/Networking/img/bd.png)

### 3.3 Digital Signals

变化是离散的，就是**数字信号**。

![img](Lecture%20Notes/Networking/img/ds.png)
...
![img](Lecture%20Notes/Networking/img/dc.png)
...
![img](Lecture%20Notes/Networking/img/wc.png)
...
![img](Lecture%20Notes/Networking/img/2n.png)

可见，确实形状挺像。但是你要说很像还远远谈不上。所以我们看看带宽变大是个啥样子：

![img](Lecture%20Notes/Networking/img/gd.png)
...
![img](Lecture%20Notes/Networking/img/xq.png)
...
![img](Lecture%20Notes/Networking/img/modem.png)

### 3.4 Transmission Impairment

#### 3.4.1 Attenuation

![img](Lecture%20Notes/Networking/img/at.png)
...
![img](Lecture%20Notes/Networking/img/sf.png)

#### 3.4.2 Distortion

![img](Lecture%20Notes/Networking/img/distortion.png)
...
![img](Lecture%20Notes/Networking/img/bdp.png)
...
![img](Lecture%20Notes/Networking/img/r.png)
...
>
> ![img](Lecture%20Notes/Networking/img/los.png)

#### 4.1.2 Line Coding Schemes

现在就是要讲到底我怎么给电平赋予意义，让啥是1，啥是0？下图是总体的策略：

![img](Lecture%20Notes/Networking/img/lcs.png)

##### 4.1.2.1 Unipolar Scheme

![img](Lecture%20Notes/Networking/img/us.png)

这种方式功率非常高，所以现在几乎不用了。

##### 4.1.2.2 Polar Schemes

![img](Lecture%20Notes/Networking/img/ps.png)
...
![img](Lecture%20Notes/Networking/img/nr.png)
...
#question/class 书上给的是500 kbaud和500 kHz，我强烈怀疑书上错了？

---

为了解决上面的时钟同步问题，我们发明了一种归零码(**Return-to-Zero, RZ**)：

![img](Lecture%20Notes/Networking/img/rz.png)
...
![img](Lecture%20Notes/Networking/img/man.png)
...
![img](Lecture%20Notes/Networking/img/ami.png)
...
![[Lecture Notes/Networking/img/r1.png|200]]

而我们似乎很少讨论诸如：3个信号元素携带4个bit的情况。

![[Lecture Notes/Networking/img/r2.png|200]]
...
>
  ![[Lecture Notes/Networking/img/r3.png|200]]
>
> * **这一段要么是0，要么是1。**
>
...
![img](Lecture%20Notes/Networking/img/2b1q.png)

然后是8B6T，举一反三即可。

![img](Lecture%20Notes/Networking/img/8b6t.png)
...
![img](Lecture%20Notes/Networking/img/810.png)

#### 4.1.4 Scrambling

先来复习一下AMI码：

![img](Lecture%20Notes/Networking/img/ami2.png)

可以看到，它在长连0的时候一直不动，所以我们要让它动，就是靠**扰码**。

![img](Lecture%20Notes/Networking/img/sc.png)
...
![img](Lecture%20Notes/Networking/img/sc2.png)|![img](Lecture%20Notes/Networking/img/sc3.png)
...
![img](Lecture%20Notes/Networking/img/sc4.png)
...
![img](Lecture%20Notes/Networking/img/hdb.png)
...
![img](Lecture%20Notes/Networking/img/atd.png)
...
>
> ![img](Lecture%20Notes/Networking/img/cy.png)

然后是量化和编码的例子：

![img](Lecture%20Notes/Networking/img/qe.png)
...
![img](Lecture%20Notes/Networking/img/lh.png)
...
![img](Lecture%20Notes/Networking/img/hf.png)

### 4.3 Transmission Modes

信号传输的过程，可以分为**并行**和**串行**。而串行又分为同步、异步、等时。

![img](Lecture%20Notes/Networking/img/tm.png)
...
![img](Lecture%20Notes/Networking/img/pt.png)
...
![img](Lecture%20Notes/Networking/img/st.png)
...
![img](Lecture%20Notes/Networking/img/ast.png)
...
![img](Lecture%20Notes/Networking/img/syt.png)
...
![img](Lecture%20Notes/Networking/img/dta.png)
...
![img](Lecture%20Notes/Networking/img/qam.png)
...
![img](Lecture%20Notes/Networking/img/bask.png)
...
![img](Lecture%20Notes/Networking/img/ib.png)
...
![img](Lecture%20Notes/Networking/img/qsg.png)
...
![img](Lecture%20Notes/Networking/img/fsk.png)
...
![img](Lecture%20Notes/Networking/img/fb.png)

#### 5.1.3 Phase Shift Keying

**Binary PSK**

![img](Lecture%20Notes/Networking/img/psk.png)

#### 5.1.5 Quadrature Amplitude Modulation

我们怎么把相位和幅度都不同的正弦波区分开呢？使用的是星座图(Constellation Diagram)。

![img](Lecture%20Notes/Networking/img/cd.png)
...
![img](Lecture%20Notes/Networking/img/gb.png)
...
![img](Lecture%20Notes/Networking/img/tdm.png)

将这个过程再深入一下：

![img](Lecture%20Notes/Networking/img/tdm2.png)
...
![img](Lecture%20Notes/Networking/img/tdm3.png)

其中最重要的就是让这两个转的球球同步起来，这样才能正常发送和接收。

既然同步时钟那么重要，如果某个数据是空的该怎么办？TDM的解决思路就是：不管有没有数据，我都把你的那份时间给你。

![img](Lecture%20Notes/Networking/img/tdm4.png)

其他的知识点看图就理解了

![img](Lecture%20Notes/Networking/img/tdm5.png)

![img](Lecture%20Notes/Networking/img/tdm6.png)

![img](Lecture%20Notes/Networking/img/tdm7.png)

![img](Lecture%20Notes/Networking/img/tdm8.png)

#example ![img](Lecture%20Notes/Networking/img/tdm9.png)

#poe 考试会考的是，这个分层级的TDM，就像FDM一样：

![img](Lecture%20Notes/Networking/img/tdm10.png)

![img](Lecture%20Notes/Networking/img/tdm11.png)
...
![img](Lecture%20Notes/Networking/img/fs.png)

#question/class 为什么T-1能携带8000个Frame?

### 6.4 Spread Spectrum

扩频就是把带宽小的变大，这样数据率也就高了。

![img](Lecture%20Notes/Networking/img/ss.png)

## 7. Transmission Media

之前提到过，在两个物理层之间还存在着一个Transmission Media，这才是真正传输物理意义上的信息的位置。

![img](Lecture%20Notes/Networking/img/tm1.png)

Transmission Media可以分为有线和无线的，其中又有更多分类。

![img](Lecture%20Notes/Networking/img/tmfl.png)

**双绞线(Twisted-Pair Cable)**

![img](Lecture%20Notes/Networking/img/tpc.png)

传的是**差分信号**，这样抗噪声能力强。就算有噪声，它对这两根线的影响也差不多，在接收方一减就能够把噪声减掉。

还可以在上面加上屏蔽罩：

![img](Lecture%20Notes/Networking/img/stp.png)

---

**同轴电缆(Coaxial Cable)**

![img](Lecture%20Notes/Networking/img/cc.png)
...
![img](Lecture%20Notes/Networking/img/gi.png)
...
![img](Lecture%20Notes/Networking/img/sm.png)
...
![img](Lecture%20Notes/Networking/img/sn.png)

而这种类型的网络也可以继续细分，这章就是分别去介绍它们。

![img](Lecture%20Notes/Networking/img/swf.png)
...
![img](Lecture%20Notes/Networking/img/dh.png)
...
![img](Lecture%20Notes/Networking/img/csn.png)
...
![img](Lecture%20Notes/Networking/img/jhjl.png)
...
![img](Lecture%20Notes/Networking/img/dn.png) ^4370c8
...
![img](Lecture%20Notes/Networking/img/rt.png)
...
![img](Lecture%20Notes/Networking/img/vn.png)
...
![img](Lecture%20Notes/Networking/img/vci1.png)
...
![img](Lecture%20Notes/Networking/img/vci2.png)
...
![img](Lecture%20Notes/Networking/img/vci3.png)

#question/class 这里的VCI和之后链路层的[[#14.2 Addressing|MAC地址]]有什么区别？MAC地址在hop-to-hop传输的时候不会改变，而这个VCI在hop-to-hop传输的时候也不会改变，VCI是不是MAC的一种呢？
...
![img](Lecture%20Notes/Networking/img/dhxl.png)

用什么传解决了，接下来是怎么传？我们的数据通常是数字信号，而这里却不是从0频率开始的，自然就能想到用第5章介绍的模拟信号来传。同时也会使用调制解调器。

![img](Lecture%20Notes/Networking/img/dhcsj.png)
...
> #question/class 这里我认为第5章说转换是不够严谨的，因为这只是一种用模拟信号来编码数字信号的方式，并不是真正的本质的转换。不过编码和转换，有时候还真可以认为是等价的。
...
![img](Lecture%20Notes/Networking/img/atb.png)
...
![img](Lecture%20Notes/Networking/img/nal.png)
...
![img](Lecture%20Notes/Networking/img/jd.png)
...
![img](Lecture%20Notes/Networking/img/mac.png)
...
![img](Lecture%20Notes/Networking/img/toe.png)
...
![img](Lecture%20Notes/Networking/img/xor.png)
...
![img](Lecture%20Notes/Networking/img/hdd.png)

![img](Lecture%20Notes/Networking/img/hdd2.png)
...
![[Lecture Notes/Networking/img/crc.png|300]]

![[Lecture Notes/Networking/img/crc2.png|300]]

> 这里笔记中有错误，模2除法的意思是使用模2减法，也就是**不借位的减法**，这里和异或操作正好是相对应的。

![[Lecture Notes/Networking/img/crc3.png|300]]
...
![img](Lecture%20Notes/Networking/img/cop.png)
...
![img](Lecture%20Notes/Networking/img/bs.png)

能看到，数据中有Flag，还有一个ESC。所以数据链路层会进行这样的打包：

![img](Lecture%20Notes/Networking/img/bs2.png)

这样，在接收方接到之后，就能正确地将数据取出来：

![img](Lecture%20Notes/Networking/img/bs3.png)

#### 12.1.2 Bit-oriented Framing

有了上面的解释，面向bit的打包方式就很简单了：

![img](Lecture%20Notes/Networking/img/bo.png)
...
![img](Lecture%20Notes/Networking/img/bof.png)
...
![img](Lecture%20Notes/Networking/img/ack.png)
...
![img](Lecture%20Notes/Networking/img/arq.png)
...
![img](Lecture%20Notes/Networking/img/snan.png)
...
![[Lecture Notes/Networking/resources/Pasted image 20221107195213.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221107195928.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221107202957.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221107204105.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221107204652.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221107205326.png]]
...
在NRM中，分为Primary Station(主站)和Secondary Station(从站)。它们之间的关系，在[[Lecture Notes/Operating System/os#8.3 Hardwares|操作系统的笔记中8.3]]介绍Terminal的时候提到过。以前因为计算机很昂贵，都是多人使用一个。这个计算机建在机房中，而每个用户只有一个键盘鼠标显示器，并没有CPU之类的。这些东西组成一个终端，通过远程去访问那个昂贵的计算机。这个计算机就是主站，而用户的键鼠就是从站。

主站所发出的Frame叫做Command，而从站发出的Frame叫做Response。因此它们之间的通信可以是p2p，也可以是Multipoint Links：

![img](Lecture%20Notes/Networking/img/nrm.png)
...
![img](Lecture%20Notes/Networking/img/abm.png)
...
* Information Frame
  ...
  ![img](Lecture%20Notes/Networking/img/if.png)
...
* Supervisory Frame
  ...
  ![img](Lecture%20Notes/Networking/img/sff.png)
...
* Unnumbered Frame
  ...
  ![img](Lecture%20Notes/Networking/img/uf.png)

如何区分这三种Frame呢？根据**Control段**的前两个bit就可以：

![img](Lecture%20Notes/Networking/img/isu.png)
...
![img](Lecture%20Notes/Networking/img/pppf.png)
...
![img](Lecture%20Notes/Networking/img/macc.png)
...
![img](Lecture%20Notes/Networking/img/coll.png)
...
![img](Lecture%20Notes/Networking/img/aloha.png)
...
![[Lecture Notes/Networking/resources/Pasted image 20221109170727.png]]
...
![img](Lecture%20Notes/Networking/img/sa.png)
...
![img](Lecture%20Notes/Networking/img/pz.png)

所以，对于一个CSMA的网络，它有一个Vulnerable Time，也就是会发生碰撞的时间。这个时间自然就是propagation time了。在这段时间内如果有的结点要发frame，就有可能产生碰撞。

![img](Lecture%20Notes/Networking/img/vt.png)

#question/class 这个监听到底是什么机制？是隔一段时间监听一下；还是一直在监听，只是听一段时间发现没人就说话？按着这个脆弱时间的解释，我觉得应该是后者。
...
![img](Lecture%20Notes/Networking/img/1p.png)
...
![img](Lecture%20Notes/Networking/img/np.png)
...
![img](Lecture%20Notes/Networking/img/pp.png)

![img](Lecture%20Notes/Networking/img/pp2.png)
...
![img](Lecture%20Notes/Networking/img/cscd.png)
...
![img](Lecture%20Notes/Networking/img/csmacd.png)
...
![[Lecture Notes/Networking/resources/Pasted image 20221029113404.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221029122158.png]]

![[Lecture Notes/Networking/resources/Pasted image 20221029122221.png|500]]

![[Lecture Notes/Networking/resources/Pasted image 20221029122210.png]]
...
![img](Lecture%20Notes/Networking/img/802.png)

而它们也将Data-link layer分成了两层，就像[[#10.3 DLC and MAC|10.3]]中提到的一样：

![img](Lecture%20Notes/Networking/img/lm.png)
...
![img](Lecture%20Notes/Networking/img/8f.png)

* Preamble: 在物理层加的，严格上讲并不是frame的一部分。这里是56bit的0和1，用来同步时钟。另外我们在操作系统中也提过[[Lecture Notes/Operating System/os#^c31505|类似的概念]]。
...
![img](Lecture%20Notes/Networking/img/um.png)
...
![img](Lecture%20Notes/Networking/img/cb.png)
...
![img](Lecture%20Notes/Networking/img/oe5.png)
...
![img](Lecture%20Notes/Networking/img/cod.png)
...
![[Lecture Notes/Networking/resources/Pasted image 20221014195317.png]]

而如今的网络通常是好多个结点互相传播，因此在这些结点中间布了一个中间结点，它也有Repeater的作用，同时也能够给所有结点建立连接，这就是**Hub**：

![[Lecture Notes/Networking/resources/Pasted image 20221014195549.png]]

Hub的内部结构是这样的：

![[Lecture Notes/Networking/resources/Pasted image 20221014195814.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221014201116.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221014202442.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221028124525.png|600]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221028131150.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221028132358.png|600]]

* 之后，运用你学习的算法，就能够生成一颗最小的树了：

![[Lecture Notes/Networking/resources/Pasted image 20221028132656.png|600]]

在这棵树上进行通信，就不会发生之前的绕圈圈的情况。但是，注意一开始说的：**从逻辑上切断。**其实，这根线是没有切断的，只不过我们用软件将帧的传输给阻塞了：

![[Lecture Notes/Networking/resources/Pasted image 20221028132957.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221028135753.png]]

甚至我还可以让交换机也分工，这样的系统就更加庞大了：

![[Lecture Notes/Networking/resources/Pasted image 20221028135845.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221028141159.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221029102620.png]]
可以看到，无线局域网中并没有链路层的交换机，这也是我们现在很少看到它的原因。
* 有线网和无线网都是靠路由器去和其他的网络连接。但是也是有区别的：
![[Lecture Notes/Networking/resources/Pasted image 20221029103038.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221029105252.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221029105935.png]]

### 16.2 MAC Sublayer

还是回到[[#10.3 DLC and MAC|10.3]]中的这个MAC，在IEEE的802.11标准中，将这个层又给拆开了：

![[Lecture Notes/Networking/resources/Pasted image 20221029110527.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221029111559.png|300]] ![[Lecture Notes/Networking/resources/Pasted image 20221029111638.png|300]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221029143245.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221029143714.png]]

### 17.1 IPv4 Address

这个咱实在是太熟了！一共32bit：

![[Lecture Notes/Networking/resources/Pasted image 20221029144722.png]]

IP地址被拆成了两半：前一半是网络段，后一半是主机段。网络段用于路由器去定位目标的路由器；而主机段用于接收方去寻找自己管的那个计算机。这样分很好理解，但是究竟多少是网络段，多少是主机段呢？具体的分法有5种，**也就是常听到的ABCDE 5类地址**：

![[Lecture Notes/Networking/resources/Pasted image 20221029145409.png]]

另外，在IP地址空间中，保留了几个用于**私有网络**的地址。私有网络地址通常应用与公司、组织和个人网络，它们没有置于因特网。

![[Lecture Notes/Networking/resources/Pasted image 20221029150106.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221029152305.png]]

因此，子网掩码的作用，就是**把最后的主机段置零，把前面的网络段和子网段保留下来**。这也是我们看到的子网掩码为什么通常都是`255.255.255.0`的原因了。另外，像我[[Homework/Data Management/1. hadoop#^3af7e7|作业中提到的]]这种写法叫做**变长子网掩码(Variable Length Subnetwork Mask, VLSM)**。这种方式通常是为了给子网更加细分，比如子网的子网就比子网多一位掩码。
...
![[Lecture Notes/Networking/resources/Pasted image 20221030121321.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221109194641.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221109194955.png]]

这个工作就交给路由器来处理了，而这个路由器肯定也不能这么傻乎乎地把所有地址都变成一个公共地址。它的内部设计是这样的：

![[Lecture Notes/Networking/resources/Pasted image 20221109195058.png]]

![[Lecture Notes/Networking/resources/Pasted image 20221109195128.png]]

![[Lecture Notes/Networking/resources/Pasted image 20221109195136.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221109195630.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221109200206.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221109201051.png]]

数据报看起来好像很简单啊，只有一个头和一个尾巴。其中尾巴里装的就是要实际发的数据，而头部主要存一些信息。但是这个头的内部构造可不简单：

![[Lecture Notes/Networking/resources/Pasted image 20221109201214.png]]
...
* Protocol：数据报中的数据打包的方式有非常多种，这取决于我们用什么协议。我们可以用上层提供的TCP，UDP等协议，也可以用网络层自带的协议。那么这些协议都有一个独一无二的编号，这个编号就写在这段区域里：
  ...
  ![[Lecture Notes/Networking/resources/Pasted image 20221110130805.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221110142650.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221110155659.png]]

从0号到3999号。现在我要把它切成三段，每段是个fragment，那么我们很容易就能写出这三段的编号：

![[Lecture Notes/Networking/resources/Pasted image 20221110155755.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221110160422.png]]

好了，关于Fragment的知识都介绍完了，下面给出一个详细的例子：

![[Lecture Notes/Networking/resources/Pasted image 20221110160810.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127114030.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127114441.png]]

显然，只有N2认可这个ARP请求包，然后把自己的MAC地址一填，返回给N1。返回的时候就不再是广播的了，而是unicast单播：

![[Lecture Notes/Networking/resources/Pasted image 20221127114535.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127115435.png]]
...
有了这个包，我们将他作为**Data**，添加到[[Lecture Notes/Networking/img/8f.png|链路层的Frame]]中，就可以发送了。注意在请求的时候，因为是广播地址，所以Destination address字段是全1。 ^bf222a
...
![[Lecture Notes/Networking/resources/Pasted image 20221127123336.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127123944.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127131855.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127132314.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127134132.png]]

但是，这张表中通常包含一个默认选项，也就是当收到的datagram和所有的表项都不匹配时，该往哪里发？这里能看出，只有可能是下面这个路由器，也就是180.70.65.200/26。因此写出完整的表格：

![[Lecture Notes/Networking/resources/Pasted image 20221127134251.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127161148.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127162010.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127162759.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20221127164130.png]]

> 简单来说，就是把2，也就是当前路径的花费加到A的**每一项(即使是A和B也要加)**中，看得到的结果和B的老值哪一个大，取小的作为新的B值。

另外我们还可以写一下B和E的交互，即E把自己的vector发给B后B是怎么更新自己的vector的：

![[Lecture Notes/Networking/resources/Pasted image 20221127164502.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230107151305.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230107151852.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230107152530.png]]

而且A也会傻傻地以为，这条链路的花费就是2。因此它会将这个数字加到AB之间的花费上，和无穷比较大小。显然，A会稀里糊涂地将自己到X的花费改成了3：

![[Lecture Notes/Networking/resources/Pasted image 20230107152743.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230107153154.png]]

再往后，它们不断发送vector，也不断重复着上述过程，直到他俩最终都意识到，这花费是无穷啊！

![[Lecture Notes/Networking/resources/Pasted image 20230107153235.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230108174107.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230108174419.png]]

一开始我能知道什么？当然是，节点0到它自己的最短距离，那就是0！因此，我们将节点0标记为以访问，**这也意味着我已经知道从初始节点到节点0的最短距离是多少了**。然后将节点0的花费改成0：

![[Lecture Notes/Networking/resources/Pasted image 20230108174604.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230108174930.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230108175937.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230108180329.png]]

然后，依然是选(未被访问的)最小的那条，显然就是7。因此，我们认为从0到7的最短路径就是0-1-7。将节点7标记为已访问。

![[Lecture Notes/Networking/resources/Pasted image 20230108180514.png]]

经过不断循环，最终我们就能够将表格填完：

![[Lecture Notes/Networking/resources/Pasted image 20230108180717.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230307123927.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230307131947.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230108183244.png]]

一般来说，小的端口号更加出名和常用，一般是服务器上的进程；而个人用户的进程的端口号会更大。端口号一共有16bit，所以范围是0-65535：

![[Lecture Notes/Networking/resources/Pasted image 20230108183344.png]]

下面是我们常常听到的**socket地址**。下面就是一个例子：

![[Lecture Notes/Networking/resources/Pasted image 20230108183727.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230108184505.png]]

很简单，头加上数据就是了。下面我们看看这个头里都有什么：

![[Lecture Notes/Networking/resources/Pasted image 20230109120939.png]]

![[Lecture Notes/Networking/resources/Pasted image 20230109121248.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230109121502.png]]

既然是这种方式的话，就要注意了。发送方和接收方的速率不一样，所以我们不能像UDP一样啥也不管一股脑塞过去。因此我们可以使用buffer来传递。下面是一个例子，发送方和接收方都有一个buffer，使用循环队列来实现，都是20byte：

![[Lecture Notes/Networking/resources/Pasted image 20230109122001.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230109150418.png]]

> 注意，segment的大小不一定非要是一样的。

#poe 接下来我们看一看TCP的segment到底是什么格式。它和UDP的user datagram是对应的。

![[Lecture Notes/Networking/resources/Pasted image 20230109150817.png]]

还是很简单！但是，这个头却很复杂：

![[Lecture Notes/Networking/resources/Pasted image 20230109150839.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230109152001.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230109153417.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230109175735.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230109182409.png]]

开始传的时候，第一个字节是8001号，一直到9000号。等它开始传第二个segment的时候，就是从9001号开始了。它咋知道的呢？就是用[[#^c7b399|之前我们介绍的方法]]。用第一个字节的编号加上data部分的长度，就能得到下一个segment应该从多少号开始。如果看到这里你发现了问题，那么你的思考还是比较细致的：**data的长度从哪儿来啊**？回看之前的segment的结构，并没有找到哪个部分是data长度或者总体的长度，只能找到HLEN。那么我们咋得到？实际上，这部分长度是和网络层合作算出来的。虽然segment不知道自己有多长，但是将它打包成datagram的时候，[[Lecture Notes/Networking/resources/Pasted image 20221109201214.png|datagram]]可知道自己有多长！datagram中有一个Total Length的字段，用这个长度减去datagram的头部，就能得到整个segment的长度；再用segment的长度剪掉segment的头部就可以了！
...
![[Lecture Notes/Networking/resources/Pasted image 20230109215403.png]]
...
![[Lecture Notes/Networking/resources/Pasted image 20230110122126.png]]
