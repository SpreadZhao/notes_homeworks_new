...
我们要在标出来的两个位置之前创造符号表。但是我们此时并没有机会去创造，因为按照语义规则，只有在规约出产生式左部的时候才能执行语义规则中的代码。因此，我们需要修改产生式，这也是**前人非常天才的设计**：
...
之前对于数组，我们只给了声明的例子。下面我们给一个完整的例子，看看数组到底是怎么被声明和调用的。首先是文法：
