...
从制作画图程序的实验就能够看出，语义分析通常是编译过程中最重要的环节。一句话，**语义分析就是分析出我这句话到底是要干什么**？赋值-赋给谁？赋成啥？我们学习的语义分析只是一个小方面，也就是**语法制导翻译生成中间代码**。比如下面一个产生式：
...
如何使用三地址码呢？具体的实现有**三元式**和**四元式**。我们从三元式开始说起。我们对于每一个产生式，都可以给出它的语义规则。这样**在进行Down-top的语法分析的时候，每进行一次规约，都可以按照对应的语义规则来收集信息**。下面的问题是：给什么规则？当然是生成三地址码的规则啦！我们就从三元式开始说起，看看文法是如何生成三元式的。
...
这个方法接受一个id作为参数，并返回它在符号表中的内存地址。这样，我们就可以写出最后一条产生式的语义规则了。我们让每个非终结符都有一个属性叫做code，它就表示内存地址。这样的话，最后一条产生式的语义规则就是：
...
![[Excalidraw/Drawing 2022-12-29 13.58.11.excalidraw|500]]
...
![[Excalidraw/Drawing 2022-12-29 14.09.14.excalidraw|500]]
...
![[Excalidraw/Drawing 2022-12-29 14.14.45.excalidraw|500]]

> 这里的(1)就是`trip()`函数第一次被调用返回的结果。它生成一个三元式`(1) (*, b, c)`。要注意这里的b和c还是code哦!

* 接下来就是将E+E规约成E了，并按照第二条产生式的语义规则继续生成三元式：

![[Excalidraw/Drawing 2022-12-29 14.18.33.excalidraw|500]]
...
![[Excalidraw/Drawing 2022-12-31 17.03.28.excalidraw]]

首先要规约的就是这个int，将它规约成T之后，就知道了这个T的type和width：

![[Excalidraw/Drawing 2022-12-31 17.14.06.excalidraw]]

之后就是将array的部分总共再规约成一个T，那么此时也就是调用`array()`函数来构建整个数组的信息了；另外下一步是将最右边的int规约成T，就一起做了。

![[Excalidraw/Drawing 2022-12-31 17.17.39.excalidraw]]

下一步是将id:T规约成D，那么这个时候终于用到第二条产生式了！此时就真正开始填表操作：

![[Excalidraw/Drawing 2022-12-31 17.21.42.excalidraw]]
...
> 这里其实就已经解决了之前那个[[#^9d998b|最隐蔽的问题]]。我们给父亲生了个儿子，并且儿子能拿到父亲这个参数，所以只需要给儿子开辟一个成员，让这个指针指向父亲，就已经实现了儿子符号表访问父亲符号表。

第二条的语义规则中传入的是`null`，这代表这它没有父亲，是最底层的符号表。
...
![[Excalidraw/Drawing 2023-01-03 11.05.48.excalidraw|700]]
...
![[Excalidraw/Drawing 2023-01-03 11.58.37.excalidraw|700]]
...
![[Excalidraw/Drawing 2023-01-06 15.38.22.excalidraw|500]]
...
![[Excalidraw/Drawing 2023-01-06 21.13.21.excalidraw|700]]

前几步都是正常的规约，这里就直接跳过了：

![[Excalidraw/Drawing 2023-01-06 21.46.16.excalidraw|700]]

此时我们要将arr\[E规约成E'，那么就用到了第四条产生式的语义规则了。此时记住我们最重要的目的：**算v1**！其他所有的操作，其实都是在为我们后续算v服务。按着语义规则走下去，我们能得到如下结果：

![[Excalidraw/Drawing 2023-01-06 21.54.36.excalidraw|700]]

v是在place属性中存的，所以我们每次最重要的结果就是得到新的规约出来的东西的place属性。接下来是规约j+y的过程，也直接跳过了：

![[Excalidraw/Drawing 2023-01-06 21.57.47.excalidraw|700]]

之后就是将E', E规约成E'了。这次规约也是本次语义分析中最重要的一次。按着第五条产生式的语义规则，我在每条语句后面都表上当前的执行结果：
...
![[Excalidraw/Drawing 2023-01-06 22.04.41.excalidraw|700]]
...
![[Excalidraw/Drawing 2023-01-06 22.14.22.excalidraw|700]]