...
#keypoint 这里的表述有问题，同时我下面的例子中也有问题，虽然结果都是正确的。最左直接短语并不是先最下面后最左边，我们要按着感觉来找，**从左往右数第一个能规约的东西**才是最左直接短语(句柄)。
...
生成的东西其实一模一样，唯一的区别是，**在生成的过程中，需要分配一块空间，这块空间其实就是t1, t2, t3这些变量的位置，然后E的code属性要指向这些空间**。
...
> 这里其实是一个快排的例子，只不过已经简化的看不出来是排序了。我们只保留了变量的声明部分，并且忽略了函数的参数。
...
这有问题吗？当然没有！变量`a`是父结构`sort()`中声明的，自然可以在子结构`readarray()`中使用。所以，我们还要实现**让子结构的符号表能通过某种机制访问到父结构的符号表**。关子就不卖了，我们直接给语义规则：
...
3 | D -> D;D | ---
4 | D -> id:T | `enter(tblptr.top(), id.name, T.type, top(offset)); top(offset) += T.width;`
